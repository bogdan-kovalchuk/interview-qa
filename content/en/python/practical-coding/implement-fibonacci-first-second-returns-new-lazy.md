---
id: py-prac-0016
title: "Implement `fibonacci(n, first=0, second=1)` that returns a new lazy iterator on every call and yields exactly n values with O(1) state."
description: "Implement `fibonacci(n, first=0, second=1)` that returns a new lazy iterator on every call and yields exactly n values with O(1) state."
track: python
