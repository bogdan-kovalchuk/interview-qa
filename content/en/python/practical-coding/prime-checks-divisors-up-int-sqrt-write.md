---
id: py-prac-0022
title: "For `is_prime(n)`, which checks divisors up to `int(sqrt(n))`, write tests that kill the mutants `<` vs `<=` at the square boundary and the bug for `n < 2`."
description: "For `is_prime(n)`, which checks divisors up to `int(sqrt(n))`, write tests that kill the mutants `<` vs `<=` at the square boundary and the bug for `n < 2`."
track: python
