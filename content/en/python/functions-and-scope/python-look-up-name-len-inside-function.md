---
id: py-funcs-0014
title: "How will Python look up the name `len` inside a function if there is no local binding, but the module defines its own `len`?"
description: "How will Python look up the name `len` inside a function if there is no local binding, but the module defines its own `len`?"
track: python
