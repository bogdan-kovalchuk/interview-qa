...
<span class="warn">У C аргументи передаються за значенням</span> – зміна локальної копії `state` зникне після повернення, і перехід стану загубиться. FSM тут означає finite state machine. 
