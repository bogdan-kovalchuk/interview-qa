---
id: eng-vcs-0002
title: "Чим merge commit відрізняється від rebase за history shape та commit identities?"
description: "git merge зберігає розгалужену топологію історії та оригінальні SHA комітів обох гілок, тоді як git rebase переписує коміти з новими SHA, роблячи історію лінійною."
track: engineering
...
sources:
  ...
  - source_id: predecessor-answer
    ...
    url: https://github.com/tavor118/pj_python_interview_questions_and_answers/blob/02d57a7a9f34fd386eb8aa5c0094fe3f3c3ba141/docs/development/git.md#L165-L204
    accessed: 2026-09-04
    kind: community
    version: null
...
**`git merge` зберігає розгалужену топологію історії та оригінальні SHA комітів обох гілок, тоді як `git rebase` переписує коміти з новими SHA, роблячи історію лінійною.**[^git-git-merge] Merge commit має два parent commits і фіксує момент злиття, не змінюючи жодного попереднього коміту. Rebase натомість послідовно застосовує коміти на новий base (подібно до cherry-pick), тому навіть ідентичний за змістом коміт отримує новий хеш через зміну parent.
