---
id: py-perf-0005
title: "Як перевірити, що asymptotically кращий algorithm реально виграє на representative input sizes та distributions?"
description: "Побудувати бенчмарк на розмірах, близьких до продакшн, з різними розподілами вхідних даних і повторними вимірами через timeit, потім порівняти абсолютний час та кут зростання latency."
track: python
section: performance
level: middle
type: practical
...
**Побудувати бенчмарк на розмірах, близьких до продакшн, з різними розподілами вхідних даних і повторними вимірами через `timeit`, потім порівняти абсолютний час та кут зростання latency.**[^py314-library-profile] Теоретично краща складність (наприклад, `O(n log n)` проти `O(n²)`) може програвати на малих `n` через більші constants. Потрібно тестувати на різних розподілах – випадкові, майже відсортовані, з дублікатами – і перевіряти, чи збігається емпіричне зростання з теоретичним. <span class="warn">Без representative input sizes «кращий» алгоритм може виявитися повільнішим на реальному workload.</span>
