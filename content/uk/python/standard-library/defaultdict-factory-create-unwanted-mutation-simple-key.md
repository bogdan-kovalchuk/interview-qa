---
id: py-stdlib-0003
title: "Коли factory у `defaultdict` може створити небажану mutation при простому читанні key?"
description: "Будь-яке звернення через d[key] для відсутнього ключа викликає default_factory і додає новий запис у словник, навіть якщо ви лише «читали» значення."
track: python
...
**Будь-яке звернення через `d[key]` для відсутнього ключа викликає `default_factory` і додає новий запис у словник, навіть якщо ви лише «читали» значення.**[^py314-library-collections] Наприклад, `if d[key]:` для ще не існуючого ключа створить entry з дефолтним значенням. Метод `get()` не викликає factory і не мутує словник – використовуйте його для безпечного читання.
