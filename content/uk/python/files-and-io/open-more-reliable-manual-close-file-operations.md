---
id: py-fileio-0006
title: "Чому `with open(...)` надійніше за ручний `close()` після операцій із файлом?"
description: "with (context manager) гарантує виклик close() навіть якщо всередині блоку виникне виняток."
track: python
section: files-and-io
level: middle
type: mechanism
tags: [with-open, close]
status: published
updated: 2026-09-04
content_revision: 1
reconciled_with:
...
sources:
  ...
  - source_id: predecessor-answer
    ...
    url: https://github.com/tavor118/pj_python_interview_questions_and_answers/blob/02d57a7a9f34fd386eb8aa5c0094fe3f3c3ba141/docs/python/files_and_io.md#L93-L98
    accessed: 2026-09-04
    kind: community
    version: null
...
**`with` (context manager) гарантує виклик `close()` навіть якщо всередині блоку виникне виняток.**[^py314-library-io] При ручному `f = open(...); ...; f.close()` виняток між `open()` і `close()` залишить файловий дескриптор відкритим, що може призвести до витоку ресурсів і перевищення ліміту дескрипторів ОС. `with` викликає `__exit__`, який закриває файл у будь-якому випадку.
