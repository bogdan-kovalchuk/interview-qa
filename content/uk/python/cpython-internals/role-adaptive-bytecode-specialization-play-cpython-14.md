...
**Adaptive specialization замінює generic опкоди на спеціалізовані варіанти, оптимізовані під конкретні типи аргументів, використовуючи inline cache entries для зберігання runtime-інформації.**[^py314-library-dis] Коли інтерпретатор бачить, що опкод часто виконується з певними типами, він «адаптується» – наприклад, `BINARY_OP` може стати спеціалізованою інструкцією для int+int. Якщо припущення перестає виконуватися, опкод деградую до базової версії (`baseopcode`). У 3.14 додано CLI-опцію `-S` (`--specialized`) для перегляду спеціалізованого bytecode.
