...
from .verify_build import main as run_verify
...
def run(root: Path) -> int:
    ...
    if npm is None:
        ...
        return 1
    # site/scripts/build.mjs shells out to `iqa.mirror`; hand it this interpreter
    # so the mirror is generated by the same Python that just validated and
    # exported, not by whatever a bare `python` resolves to in npm's PATH.
    site_env = {**os.environ, "IQA_PYTHON": sys.executable}
    site_build = subprocess.run([npm, "run", "build"], cwd=str(root / "site"), env=site_env)
    if site_build.returncode != 0:
        print("Site build failed.", file=sys.stderr)
        return site_build.returncode

    print("\n== 4/4: verify build ==")
    return run_verify(["--root", str(root)])
