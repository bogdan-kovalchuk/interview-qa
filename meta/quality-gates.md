...
| `sections` | усі обов'язкові секції у правильному порядку; секції – рівно `##`, підблоки `Evaluation guide` – рівно `###`, інших `##` немає; необов'язкова `Question code` – або відсутня, або рівно один блок коду й жодного тексту довкола, і ніколи `TODO` |
...
| `short-answer-limits` | 2–5 речень; понад 90 слів – м'яке попередження; без заголовків і готових URL (пошук URL і Markdown-посилань ігнорує код – див. нижче). Лічильники не залежать від форми розмітки: межа речень рахується однаково для `<code>x</code>`, `` `x` ``, `<span class="key">` і `**…**`, а лічильник слів відкидає HTML-теги, але лишає їхній видимий текст. `<` у коді не читається як початок тегу |
| `sources` | ≥1 джерело; якщо всі `kind: community` – відхилити |
| `facets-vocabulary` | `level`, `type`, `status`, `frameworks` – тільки зі словників |
| `xref` | кожен токен `qid:` резолвиться; готових URL у контенті немає (пошук ігнорує код – див. нижче) |
...
**Пошук готових URL не бачить коду.** І `short-answer-limits`, і `xref` шукають `[текст](ціль)` та голий `http://` у тексті, з якого вирізано **і** fenced-блоки, **і** інлайн-код у бектиках. Інакше `` `table[opcode](ctx, frame)` `` – звичайний масив вказівників на функції в C – читається як Markdown-посилання, і легальний запис стає неможливим. Підрахунок речень і слів навмисно лишається на тексті **з** інлайн-кодом: термін у бектиках читач усе одно читає.
...
виміряно на робочій Windows-машині: локальний запуск `iqa build` періодично ловить `WinError 5`,
бо файловий вотчер – VS Code чи антивірус – тримає щойно створений файл частку секунди. Обидві
операції над деревом дзеркала на це наражаються, і обидві тепер ретраяться в `tools/iqa/mirror.py`:
`remove_tree` (видалення попереднього дзеркала) і `swap_tree` (перейменування щойно записаного
тимчасового дерева на місце). Другий випадок знайдено пізніше за перший і він відтворювався **з
порожнім призначенням**, тобто ретрай на видаленні його не покривав. У CI кожен запуск – чистий
