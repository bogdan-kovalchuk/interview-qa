...
| `short-answer-limits` | 2–5 речень; понад 90 слів – м'яке попередження; без заголовків і готових URL (пошук URL і Markdown-посилань ігнорує код – див. нижче). Лічильник речень не залежить від форми розмітки: межа рахується однаково для `<code>x</code>`, `` `x` ``, `<span class="key">` і `**…**`, а `<` у коді не читається як початок тегу |
